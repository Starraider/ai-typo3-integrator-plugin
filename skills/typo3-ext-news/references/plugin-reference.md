...
Confirm plugin CTypes, controller actions, template names, and TypoScript
settings against the installed `georgringer/news` version before copying a
pattern. The entries below are implementation aids, not a substitute for local
package evidence.
